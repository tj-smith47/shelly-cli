...
func TestNewCommand_Structure(t *testing.T) {
	...
	if cmd.Use != "sleep <device>" {
		t.Errorf("Use = %q, want %q", cmd.Use, "sleep <device>")
	}

	wantAliases := []string{"goodnight", "nap"}
...
func TestRun_ShortDelay(t *testing.T) {
	...
	out := tf.OutString()
	if !strings.Contains(out, "Goodnight") && !strings.Contains(out, "off") {
