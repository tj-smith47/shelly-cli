...
func TestNewCommand_Structure(t *testing.T) {
	...
	if cmd.Use != "remove <device> <component>" {
		t.Errorf("Use = %q, want %q", cmd.Use, "remove <device> <component>")
	}

	wantAliases := []string{"rm", "del", "delete"}
...
func TestNewCommand_Args(t *testing.T) {
	...
	tests := []struct {
		...
		{"two args valid", []string{"device", "temperature:100"}, false},
		{"three args", []string{"device", "component", "extra"}, true},
	}
...
func TestOptions(t *testing.T) {
	...
	if opts.Component != "temperature:100" {
		t.Errorf("Component = %q, want %q", opts.Component, "temperature:100")
	}
...
func TestExecute_WithMock(t *testing.T) {
	...
	cmd.SetArgs([]string{"test-device", "temperature:100", "--yes"})
	cmd.SetOut(&buf)
	cmd.SetErr(&buf)
...
func TestExecute_DeviceNotFound(t *testing.T) {
	...
	cmd := NewCommand(tf.Factory)
	cmd.SetContext(context.Background())
	cmd.SetArgs([]string{"nonexistent-device", "temperature:100", "--yes"})
	cmd.SetOut(&buf)
	cmd.SetErr(&buf)
...
func TestExecute_Cancelled(t *testing.T) {
	...
	cmd := NewCommand(tf.Factory)
	cmd.SetContext(context.Background())
	cmd.SetArgs([]string{"test-device", "temperature:100"})
	cmd.SetOut(&buf)
	cmd.SetErr(&buf)
