...
func TestNewCommand_Structure(t *testing.T) {
	...
	// Test Aliases
	wantAliases := []string{"turn-on", "enable"}
...
func TestRun_Gen2Switch(t *testing.T) {
	...
	out := tf.OutString()
	if !strings.Contains(out, "turned on") {
		t.Errorf("Output should contain 'turned on', got: %s", out)
	}
}
	...
	fixtures := &mock.Fixtures{
		...
		DeviceStates: map[string]mock.DeviceState{
			"multi-switch": {
				...
				"switch:1": map[string]any{"output": false},
			},
		},
	}
