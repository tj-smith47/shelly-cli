...
func TestNewCommand_Structure(t *testing.T) {
	...
	if cmd.Use != "prev" {
		t.Errorf("Use = %q, want %q", cmd.Use, "prev")
	}

	wantAliases := []string{"previous", "p"}
	if len(cmd.Aliases) != len(wantAliases) || cmd.Aliases[0] != wantAliases[0] || cmd.Aliases[1] != wantAliases[1] {
...
func TestRun_CyclesToPrevTheme(t *testing.T) {
