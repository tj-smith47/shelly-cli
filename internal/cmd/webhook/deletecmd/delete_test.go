...
func TestNewCommand_Structure(t *testing.T) {
	...
	if cmd.Use != "delete <device> <webhook-id>" {
		t.Errorf("Use = %q, want %q", cmd.Use, "delete <device> <webhook-id>")
	}
...
func TestNewCommand_ValidArgsFunction(t *testing.T) {
	...
	// Factory-based commands may or may not have ValidArgsFunction
	_ = cmd.ValidArgsFunction // Just access it to ensure no panic
}
...
func TestExecute_WithMock(t *testing.T) {
	...
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"test-device", "1", "-y"})
...
func TestExecute_DeviceNotFound(t *testing.T) {
	...
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"nonexistent-device", "1", "-y"})
