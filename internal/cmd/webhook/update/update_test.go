...
func TestNewCommand_Structure(t *testing.T) {
	...
	if cmd.Use != "update <device> <webhook-id>" {
		t.Errorf("Use = %q, want %q", cmd.Use, "update <device> <webhook-id>")
	}

	wantAliases := []string{"edit", "modify"}
...
func TestNewCommand_InvalidWebhookID(t *testing.T) {
	t.Parallel()
	...
	cmd := NewCommand(f)
	cmd.SetArgs([]string{"device", "not-a-number", "--enable"})
	cmd.SetOut(out)
	cmd.SetErr(errOut)
	...
	if !strings.Contains(err.Error(), "invalid webhook ID") {
		t.Errorf("expected 'invalid webhook ID' error, got: %v", err)
	}
}

func TestNewCommand_NoOptions(t *testing.T) {
	t.Parallel()
	...
	err := cmd.Execute()
	if err == nil {
		t.Fatal("expected error when no options specified")
	}

	if !strings.Contains(err.Error(), "specify at least one option") {
		t.Errorf("expected 'specify at least one option' error, got: %v", err)
	}
}
...
func TestExecute_DeviceNotFound(t *testing.T) {
	...
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"nonexistent-device", "1", "--enable"})
