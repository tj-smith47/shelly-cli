...
func TestNewCommand_Aliases(t *testing.T) {
	...
	expectedAliases := []string{"turn-off", "disable"}
...
func TestNewCommand_GroupFlag(t *testing.T) {
	...
	if err := cmd.Flags().Set("group", "bedroom"); err != nil {
	...
	if val != "bedroom" {
		t.Errorf("group = %q, want %q", val, "bedroom")
	}
}
...
func TestNewCommand_AcceptsArgs(t *testing.T) {
	...
	// off command doesn't have explicit Args validator (nil means cobra.ArbitraryArgs by default)
...
func TestRun_WithGroupFlag(t *testing.T) {
	...
	cmd := NewCommand(tf.Factory)
	cmd.SetArgs([]string{"--group", "bedroom"})
	err := cmd.Execute()
...
func TestOptions_TimeoutFlag(t *testing.T) {
	...
	if val.Seconds() != 30 {
		t.Errorf("timeout = %v, want 30s", val)
	}
}
