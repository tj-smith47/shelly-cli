...
func TestOptions(t *testing.T) {
	...
	if opts.Output != "/tmp/test.json" {
		t.Errorf("Output = %q, want %q", opts.Output, "/tmp/test.json")
	}
...
func TestExecute_NoDevices(t *testing.T) {
	...
	// Should print a warning about no devices
	...
	fixtures := &mock.Fixtures{
		...
		DeviceStates: map[string]mock.DeviceState{
			"test-device": {
				...
				"sys":      map[string]any{"mac": "AA:BB:CC:DD:EE:FF"},
			},
		},
	}
...
func TestExecute_EnergyReport(t *testing.T) {
	...
	fixtures := &mock.Fixtures{
		...
		DeviceStates: map[string]mock.DeviceState{
			"test-device": {
				"switch:0": map[string]any{"output": true, "apower": 42.5},
			},
		},
	}
...
func TestRun_EnergyReport(t *testing.T) {
	...
	fixtures := &mock.Fixtures{
		...
		DeviceStates: map[string]mock.DeviceState{
			"test-device": {"switch:0": map[string]any{"output": false, "apower": 100.5}},
		},
	}
