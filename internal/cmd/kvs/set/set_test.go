...
func TestNewCommand_Structure(t *testing.T) {
	...
	// Test Aliases
	wantAliases := []string{"s", "write", "put"}
...
func TestNewCommand_Args(t *testing.T) {
	...
	tests := []struct {
		...
		{"three args valid", []string{"device", "key", "value"}, false},
		{"four args", []string{"device", "key", "value", "extra"}, true},
	}
...
func TestNewCommand_Flags(t *testing.T) {
	...
	// Test null flag
	flag := cmd.Flags().Lookup("null")
	if flag == nil {
		t.Fatal("--null flag not found")
	}
	if flag.DefValue != "false" {
		t.Errorf("--null default = %q, want %q", flag.DefValue, "false")
	}
}
...
func TestOptions(t *testing.T) {
	...
	if opts.Value != "test-value" {
		t.Errorf("Value = %q, want %q", opts.Value, "test-value")
	}
	if !opts.Null {
...
func TestNewCommand_MissingValue(t *testing.T) {
	t.Parallel()
	...
	cmd := NewCommand(f)
	cmd.SetArgs([]string{"device", "key"}) // no value and no --null
	cmd.SetOut(out)
	cmd.SetErr(errOut)
