...
func TestNewCommand_CommandName(t *testing.T) {
	...
	if cmd.Name() != "enable" {
		t.Errorf("Name() = %q, want 'enable'", cmd.Name())
	}
}
...
func TestNewCommand_UsageString(t *testing.T) {
	...
	if !strings.Contains(usage, "enable") {
...
func TestNewCommand_NoSubcommands(t *testing.T) {
	...
	if len(cmd.Commands()) > 0 {
		t.Errorf("enable command should not have subcommands, has %d", len(cmd.Commands()))
	}
}
...
func TestNewCommand_LongMentionsRemote(t *testing.T) {
	t.Parallel()

	cmd := NewCommand(cmdutil.NewFactory())

	if !strings.Contains(cmd.Long, "remote") {
		t.Error("Long description should mention remote access")
	}
}
