...
func run(ctx context.Context, opts *Options) error {
	...
	err := cmdutil.RunWithSpinner(ctx, ios, "Disabling cloud connection...", func(ctx context.Context) error {
		if setErr := svc.SetCloudEnabled(ctx, opts.Device, false); setErr != nil {
			return fmt.Errorf("failed to disable cloud: %w", setErr)
		}
