...
func runList(ctx context.Context, opts *ListOptions) error {
	...
	sensors := collectIlluminanceSensors(fullStatus)
...
func collectIlluminanceSensors(status map[string]json.RawMessage) []Status {
	var sensors []Status
	for key, raw := range status {
		if strings.HasPrefix(key, "illuminance:") {
			var s Status
