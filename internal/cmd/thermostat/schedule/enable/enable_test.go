...
func TestNewCommand_Structure(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "has use",
			checkFunc: func(c *cobra.Command) bool { return c.Use == "enable <device>" },
			wantOK:    true,
			errMsg:    "Use should be 'enable <device>'",
		},
		{
			name:      "has short",
...
func TestRun_WithMockGen2Device(t *testing.T) {
		...
		DeviceStates: map[string]mock.DeviceState{
			"thermostat-device": {
				...
				"schedule:1": map[string]any{
					"id":       float64(1),
					"enable":   false, // Disabled schedule
					"timespec": "0 0 8 * *",
				},
			},
		},
	...
	if err != nil {
		t.Errorf("Unexpected error for Gen2 device schedule enable: %v", err)
	}
