...
func TestDisplayThermostats_JSONFormat(t *testing.T) {
	t.Parallel()
	...
	err := displayThermostats(ios, thermostats, "test-gateway", formatJSON)
	if err != nil {
		t.Fatalf("displayThermostats returned error: %v", err)
	}
	...
	if !strings.Contains(output, `"enabled"`) {
		t.Error("JSON output should contain 'enabled' field")
	}
	if !strings.Contains(output, `"target_c"`) {
		t.Error("JSON output should contain 'target_c' field")
	}
}

func TestDisplayThermostats_TextFormat(t *testing.T) {
	t.Parallel()
	...
	err := displayThermostats(ios, thermostats, "test-gateway", formatText)
	if err != nil {
		t.Fatalf("displayThermostats returned error: %v", err)
	}

	// Text format uses term.DisplayThermostats which writes to ios
	// We just verify no error occurred
}

func TestDisplayThermostats_EmptyList(t *testing.T) {
	t.Parallel()
...
func TestDisplayThermostats_Formats(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			thermostats := []model.ThermostatInfo{
				{ID: 0, Enabled: true, TargetC: 20.0},
			}

			err := displayThermostats(ios, thermostats, "test-device", tt.format)
			if err != nil {
				t.Errorf("displayThermostats(%q) returned error: %v", tt.format, err)
			}
		})
	}
}

func TestDisplayThermostats_JSONPrettyPrint(t *testing.T) {
	t.Parallel()
	...
	err := displayThermostats(ios, thermostats, "gateway", formatJSON)
	if err != nil {
		t.Fatalf("displayThermostats returned error: %v", err)
	}
	...
	// Pretty printed JSON should contain indentation (2 spaces)
	if !strings.Contains(output, "  ") {
		t.Error("JSON output should be pretty-printed with indentation")
	}
}

func TestDisplayThermostats_JSONFieldValues(t *testing.T) {
	t.Parallel()
	...
	// Check specific values are in output
	if !strings.Contains(output, "42") {
		t.Error("JSON output should contain ID value 42")
	}
	if !strings.Contains(output, "23.5") {
		t.Error("JSON output should contain target_c value 23.5")
	}
	if !strings.Contains(output, "true") {
		t.Error("JSON output should contain enabled value true")
	}
}
