// Package on provides the group on subcommand for turning group members on.
package on
...
// NewCommand creates the group on command.
func NewCommand(f *cmdutil.Factory) *cobra.Command {
	opts := &Options{Factory: f}

	cmd := &cobra.Command{
		Use:     "on <group>",
		Aliases: []string{"enable", "power-on"},
		Short:   "Turn on all group members",
		Long: `Turn on every device in a group.
...
control every controllable component on each member.`,
		Example: `  # Turn on every member of a group
...
func run(ctx context.Context, opts *Options) error {
	...
	return cmdutil.RunGroupQuick(ctx, f, opts.GroupName, cmdutil.GroupActionOn, opts.ComponentIDPointer(), opts.Concurrent)
}
