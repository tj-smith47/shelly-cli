// Package off provides the group off subcommand for turning group members off.
package off
...
// NewCommand creates the group off command.
...
control every controllable component on each member.`,
		Example: `  # Turn off every member of a group
...
func run(ctx context.Context, opts *Options) error {
	...
	return cmdutil.RunGroupQuick(ctx, f, opts.GroupName, cmdutil.GroupActionOff, opts.ComponentIDPointer(), opts.Concurrent)
}
