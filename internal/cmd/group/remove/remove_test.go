...
func TestNewCommand_ShortDescription(t *testing.T) {
	...
	expected := "Remove devices from a group"
...
func TestRun_RemoveSingleDevice(t *testing.T) {
	t.Parallel()
	...
	if err := mgr.AddDeviceToGroup("living-room", "light-1"); err != nil {
		t.Fatalf("AddDeviceToGroup() error: %v", err)
	}
	if err := mgr.AddDeviceToGroup("living-room", "light-2"); err != nil {
		t.Fatalf("AddDeviceToGroup() error: %v", err)
	}
	...
	if !ok {
		t.Fatal("group not found after remove")
	}
	if len(group.Devices) != 1 {
		t.Errorf("group devices = %d, want 1", len(group.Devices))
	}
	if group.Devices[0] != "light-2" {
		t.Errorf("remaining device = %q, want light-2", group.Devices[0])
	}
}

func TestRun_RemoveMultipleDevices(t *testing.T) {
	t.Parallel()
	...
	for _, dev := range []string{"lamp-1", "lamp-2", "switch-1", "switch-2"} {
		if err := mgr.AddDeviceToGroup("office", dev); err != nil {
			t.Fatalf("AddDeviceToGroup() error: %v", err)
		}
	}
...
func TestRun_RemoveDeviceNotInGroup(t *testing.T) {
	t.Parallel()
	...
	if err.Error() != "no devices were removed" {
		t.Errorf("error = %q, want 'no devices were removed'", err.Error())
	}
}

func TestRun_PartialRemoveWithWarnings(t *testing.T) {
	t.Parallel()
	...
	if err := mgr.AddDeviceToGroup("kitchen", "light-2"); err != nil {
		t.Fatalf("AddDeviceToGroup() error: %v", err)
	}
...
func TestRun_RemoveAllDevices(t *testing.T) {
	t.Parallel()

	mgr := setupTestManager(t)
	if err := mgr.CreateGroup("garage"); err != nil {
		t.Fatalf("CreateGroup() error: %v", err)
	}
	if err := mgr.AddDeviceToGroup("garage", "light-1"); err != nil {
		t.Fatalf("AddDeviceToGroup() error: %v", err)
	}
	if err := mgr.AddDeviceToGroup("garage", "light-2"); err != nil {
		t.Fatalf("AddDeviceToGroup() error: %v", err)
	}
...
func TestRun_EmptyDeviceList(t *testing.T) {
	...
	if err := mgr.AddDeviceToGroup("test-group", "device1"); err != nil {
		t.Fatalf("AddDeviceToGroup() error: %v", err)
	}
...
func TestNewCommand_ExecuteSuccess(t *testing.T) {
	...
	if err := mgr.AddDeviceToGroup("my-group", "device1"); err != nil {
		t.Fatalf("AddDeviceToGroup() error: %v", err)
	}
	if err := mgr.AddDeviceToGroup("my-group", "device2"); err != nil {
		t.Fatalf("AddDeviceToGroup() error: %v", err)
	}
