...
func TestNewCommand_LongDescription(t *testing.T) {
	...
	tests := []struct {
		...
		{"mentions PM components", "PM"},
		{"mentions PM1 components", "PM1"},
		{"mentions power meters", "power meter"},
	}
...
func TestNewCommand_Example(t *testing.T) {
	...
	tests := []struct {
		...
		{"shows basic usage", "shelly power list"},
		{"shows JSON output", "-o json"},
		{"shows short form", "shelly power ls"},
	}
...
func TestRun_ContextCancelled(t *testing.T) {
	...
	// We need to execute through RunE directly since cmd.ExecuteContext
	// may not be available in all cobra versions
	var stdout, stderr bytes.Buffer
	cmd.SetOut(&stdout)
	cmd.SetErr(&stderr)
...
func TestRun_MultipleDeviceFormats(t *testing.T) {
	...
	tests := []struct {
		...
		{"mDNS name", "shelly1pm-123ABC.local"},
		{"short name", "lr"},
	}
