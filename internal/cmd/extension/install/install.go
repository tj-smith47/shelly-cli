...
func run(ctx context.Context, f *cmdutil.Factory, source string, force bool) error {
	...
	case strings.HasPrefix(source, "gh:") || strings.HasPrefix(source, "github:"):
		// GitHub source
		var gerr error
		localPath, cleanup, gerr = installFromGitHub(ctx, ios, source)
		if gerr != nil {
	...
	case strings.HasPrefix(source, "http://") || strings.HasPrefix(source, "https://"):
		// URL source
		var derr error
		localPath, cleanup, derr = downloadFromURL(ctx, ios, source)
		if derr != nil {
	...
	// Install
	if err := registry.Install(localPath); err != nil {
		return err
	}
...
func installFromGitHub(ctx context.Context, ios *iostreams.IOStreams, source string) (localPath string, cleanup func(), err error) {
...
func downloadFromURL(ctx context.Context, ios *iostreams.IOStreams, url string) (localPath string, cleanup func(), err error) {
	// Create temp file
