...
func TestDisplayUpdateInfo_WithNotes(t *testing.T) {
	t.Parallel()

	ios, out, _ := testIOStreams()
	DisplayUpdateInfo(ios, "1.0.0", "2.0.0", "- Bug fixes\n- New features")

	output := out.String()
	if output == "" {
		t.Error("DisplayUpdateInfo should produce output")
	}
	if !strings.Contains(output, "1.0.0") || !strings.Contains(output, "2.0.0") {
		t.Error("output should contain version info")
	}
	if !strings.Contains(output, "Bug fixes") {
		t.Error("output should contain release notes")
	}
}

func TestDisplayUpdateInfo_NoNotes(t *testing.T) {
	t.Parallel()

	ios, out, _ := testIOStreams()
	DisplayUpdateInfo(ios, "1.0.0", "2.0.0", "")
	...
	if strings.Contains(output, "Release notes") {
		t.Error("output should not contain release notes header when empty")
	}
}

func TestDisplayRollbackInfo(t *testing.T) {
	t.Parallel()

	ios, out, _ := testIOStreams()
	DisplayRollbackInfo(ios, "2.0.0", "1.0.0")

	output := out.String()
	if output == "" {
		t.Error("DisplayRollbackInfo should produce output")
	}
	if !strings.Contains(output, "Rolling back") {
		t.Error("output should mention rolling back")
	}
	if !strings.Contains(output, "2.0.0") || !strings.Contains(output, "1.0.0") {
		t.Error("output should contain version info")
	}
}
