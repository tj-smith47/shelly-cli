...
func TestListModel_ScrollerNavigation(t *testing.T) {
	...
	// Test down navigation (no scenes, should stay at 0)
...
func TestListModel_Accessors(t *testing.T) {
	...
	if m.SelectedScene() != nil {
		t.Error("SelectedScene() should return nil with no scenes")
	}

	if m.SceneCount() != 0 {
		t.Error("SceneCount() should return 0")
	}
	...
	if m.Activating() {
		t.Error("Activating() should return false initially")
	}
...
func TestDefaultListStyles(t *testing.T) {
	...
	_ = styles.ActionCount.Render("test")
